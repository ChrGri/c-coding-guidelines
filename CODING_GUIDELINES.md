...
Variables that are instances of a struct must use the `_st` suffix. The struct suffix integrates cleanly with array and pointer modifiers.

* **Struct Instance:** `_st` (Macro: `_ST`, Example: `SensorData_t config_st;`, `#define DEFAULT_DATA_ST {...}`)

* **Pointer to Struct:** `_pst` (Example: `SensorData_t *config_pst;`)

* **Array of Structs:** `_ast` (Example: `SensorData_t config_ast[5];`)

* **Array of Pointers to Structs:** `_apst` (Example: `SensorData_t *sensorList_apst[5];`)

## 7. Structs, Typedefs, and Enums

* **Struct Tags:** Use `PascalCase` and append the `_tag` suffix for the struct tag itself to prevent namespace shadowing and avoid conflicts with instance suffixes.
...
```c
// Forward declaration is unambiguous
typedef struct SensorData_tag SensorData_t;

struct SensorData_tag
{
